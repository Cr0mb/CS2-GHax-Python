...
# - Fixed an issue where the watermark would disappear when looking in certain directions.
#   This was caused by pw_module.end_drawing() being called conditionally inside the ESP rendering loop,
#   meaning if no entities were drawn (looking at empty space), end_drawing() was skipped.

# - Now, WallHack.Render() calls pw_module.end_drawing() exactly once, after processing all entities,
#   ensuring proper rendering lifecycle regardless of whether any ESP elements were drawn.

# - This guarantees that frame rendering finishes correctly every time, preventing visual glitches
#   such as the watermark vanishing or incomplete overlays when no entities are visible.

# - Added FOV changer slider to GUI (default 90)
# - Added disclaimer that FOV writes to game memory
# - Slider updates FOV value in real-time
...
class Offsets:
    try:
        offset = get("https://raw.githubusercontent.com/Cr0mb/offsets-/refs/heads/main/offsets.json").json()
        client = get("https://raw.githubusercontent.com/Cr0mb/offsets-/refs/heads/main/client_dll.json").json()
...
class WallHack:
    ...
    def GetLocalPlayerTeam(self):
        local_player_pawn = pw_module.r_int64(self.process, self.module + Offsets.dwLocalPlayerPawn)
        if local_player_pawn == 0:
            return None
        try:
            team = pw_module.r_int(self.process, local_player_pawn + Offsets.m_iTeamNum)
            return team
        except:
            return None

    def ToggleDistanceESP(self, state):
        self.distance_esp_enabled = state
    ...
    def Render(self):
        ...
        # Always render watermark first so it never disappears
        ...
        for entity in self.GetEntities():
            if not (entity.Wts(matrix) and entity.Health() > 0):
                continue
            ...
            if self.enemy_only_enabled:
                # Show only enemies
                if entity_team == local_team:
                    continue
            elif self.team_only_enabled:
                # Show only teammates
                if entity_team != local_team:
                    continue
            ...
            color = pw_module.get_color(self.box_esp_color) if entity.Team() == 2 else pw_module.get_color(self.team_esp_color)
            ...
            if self.distance_esp_enabled:
                try:
                    ...
                    scale_factor = 10  # adjust as needed based on your game's units

                    # Calculate Euclidean distance
                    dist = ((local_pos["x"] - entity_pos["x"])**2 + 
                            (local_pos["y"] - entity_pos["y"])**2 + 
                            (local_pos["z"] - entity_pos["z"])**2) ** 0.5 / scale_factor

                    feet_2d = pw_module.world_to_screen(matrix, entity_pos, 1)
                    if feet_2d:
                        dist_text = f"{int(dist)}m"
                        text_size = self.esp_font_settings["size"]
                        text_color = pw_module.get_color(self.esp_font_settings["color"])

                        # Draw distance text slightly above feet position (adjust y offset as needed)
                        pw_module.draw_text(dist_text, feet_2d["x"] - pw_module.measure_text(dist_text, text_size) / 2, feet_2d["y"] - 10, text_size, text_color)
            ...
            if self.skeletonesp:
                ...
                for bone_start, bone_end in self.BONE_CONNECTIONS:
                    ...
                    if start_pos_screen and end_pos_screen:
                        pw_module.draw_line(start_pos_screen["x"], start_pos_screen["y"], end_pos_screen["x"], end_pos_screen["y"], skeleton_color)
...
class Program:
    ...
    def create_gui(self):
        from PyQt5.QtWidgets import QGroupBox, QScrollArea, QSlider, QLabel
        ...
        esp_features = [
            ...
            # Team filters
            ("Enemy Only", self.toggle_enemy_only, self.wall.enemy_only_enabled),
            ("Team Only", self.toggle_team_only, self.wall.team_only_enabled),
        ]
        ...
        # Misc Group with FOV slider
        misc_group = QGroupBox("Misc")
        misc_layout = QVBoxLayout()
        
        disclaimer_label = QLabel("⚠️ Warning: Changing FOV writes directly to game memory.")
        disclaimer_label.setWordWrap(True)
        disclaimer_label.setStyleSheet("color: #FF5555; font-weight: bold;")
        misc_layout.addWidget(disclaimer_label)

        fov_label = QLabel("FOV: 90")
        fov_label.setAlignment(Qt.AlignCenter)
        misc_layout.addWidget(fov_label)

        fov_slider = NoWheelSlider(Qt.Horizontal)
        fov_slider.setMinimum(50)
        fov_slider.setMaximum(179)
        fov_slider.setValue(90)  # Default FOV
        misc_layout.addWidget(fov_slider)

        def on_fov_change(value):
            fov_label.setText(f"FOV: {value}")
            try:
                local_controller = pw_module.r_int64(self.process, self.module + Offsets.dwLocalPlayerController)
                if local_controller:
                    pw_module.w_int(self.process, local_controller + Offsets.m_iDesiredFOV, value)
        ...
        scroll_layout.addWidget(misc_group)  # Add Misc group with FOV slider here
